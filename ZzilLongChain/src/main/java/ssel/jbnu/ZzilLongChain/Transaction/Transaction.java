...
public class Transaction {
	...
	public boolean processTransaction(byte[] signature) {

		if (DigitalSignature.verifiySignature(signature, this.sender,this.reciepient, this.value) == false)
		{
		...
		{
			i.UTXO = ZzilLongChain.UTXOs.get(i.transactionOutputId);
		}

		// check if transaction is valid:
		if (getInputsValue() < ZzilLongChain.minimumTransaction)
		{
		...
		{
			ZzilLongChain.UTXOs.put(o.id, o);
		}
		...
		for (TransactionInput i : inputs)
		{
			...
			ZzilLongChain.UTXOs.remove(i.UTXO.id);
		}
